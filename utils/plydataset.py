...
from pc_kit import farthest_point_sample

synsetid_to_cate = {
    '02691156': 'airplane', '02773838': 'leftleg', '02801938': 'basket',
...
class bmy(Dataset):
    ...
    def get_statistics(self):
        ...
        for f in file_name_list:
            ...
            ply = PlyData.read(self.path+'/'+f)
            vtx = ply['vertex']
            pts = np.stack([vtx['x'], vtx['y'], vtx['z']], axis=-1)
            pts = farthest_point_sample(pts,2048)
    ...
    def load(self):
        ...
        def _enumerate_pointclouds(f):
            for j, pc in enumerate(f):
                ply = PlyData.read(self.path+'/'+pc)
                vtx = ply['vertex']
                pts = np.stack([vtx['x'], vtx['y'], vtx['z']], axis=-1)
                pts = farthest_point_sample(pts, 2048)
                yield torch.from_numpy(pts), j

        file_name_list = os.listdir(self.path)
        for pc, pc_id in _enumerate_pointclouds(file_name_list):
