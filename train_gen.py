...
parser.add_argument('--sample_num_points', type=int, default=4096)
...
parser.add_argument('--dataset_path', type=str, default='./data/Patches_noHole_and_collected.h5')
...
# trainloader=PUNET_Dataset(h5_file_path="D:\songda\diffusion-point-cloud-main\data\Patches_noHole_and_collected.h5",
#                           split_dir="D:\songda\diffusion-point-cloud-main\data\\train_list.txt")
# num_workers=0
train_data_loader=data.DataLoader(dataset=trainloader,batch_size=16,shuffle=True,
                                   num_workers=num_workers,pin_memory=True,drop_last=True)

val_dset = PUNET_Dataset(h5_file_path="D:\songda\diffusion-point-cloud-main\data\Patches_noHole_and_collected.h5",
                          split_dir="D:\songda\diffusion-point-cloud-main\data\\test_list.txt")
train_iter = get_data_iterator(data.DataLoader(dataset=trainloader,batch_size=16,shuffle=True,
                                    num_workers=num_workers,pin_memory=True,drop_last=True))
val_iter = get_data_iterator(data.DataLoader(dataset=val_dset,batch_size=4,shuffle=True,
                                    num_workers=num_workers,pin_memory=True,drop_last=True))
...
def validate_inspect(it):
    ...
    x = model.sample(z, args.sample_num_points,sampled_pc = None, flexibility=args.flexibility) #, truncate_std=args.truncate_std)
...
def test(it):
    ...
    for i, data in enumerate(val_dset):
        ...
        ori_pc.append(torch.from_numpy(data[0][:,0:3]).unsqueeze(0))
        ref_pcs.append(torch.from_numpy(data[1][:,0:3]).unsqueeze(0))

    ref_pcs = torch.cat(ref_pcs, dim=0)
    ori_pc = torch.cat(ori_pc, dim=0)
    ...
    for i in tqdm(range(0, math.ceil(args.test_size / args.val_batch_size)), 'Generate'):
        with torch.no_grad():
            ...
            z_mu, z_sigma = model.encoder(ori_pc.to(args.device))
            z = reparameterize_gaussian(mean=z_mu, logvar=z_sigma)
            x = model.sample(z, args.sample_num_points,sampled_pc=None, flexibility=args.flexibility)
